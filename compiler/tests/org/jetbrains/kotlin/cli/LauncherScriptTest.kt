...
class LauncherScriptTest : TestCaseWithTmpdir() {
    ...
    private fun kotlinc(vararg args: String) {
    ...
    fun testKotlinNoReflect() {
        kotlinc("$testDataDirectory/reflectionUsage.kt", "-d", tmpdir.path)
    ...
    fun testLegacyAssert() {
        kotlinc("$testDataDirectory/legacyAssertDisabled.kt", "-Xassertions=legacy", "-d", tmpdir.path)

        runProcess("kotlin", "LegacyAssertDisabledKt", "-J-da:kotlin._Assertions", workDirectory = tmpdir)

        kotlinc("$testDataDirectory/legacyAssertEnabled.kt", "-Xassertions=legacy", "-d", tmpdir.path)
...
"""
    ...
    fun testProperty() {
        kotlinc("$testDataDirectory/property.kt", "-d", tmpdir.path)
...
""")
    ...
    fun testNoClassDefFoundErrorWhenClassInDefaultPackage() {
        ...
        kotlinc("$testDataDirectory/defaultPackage.kt", "-d", testDir.path)
